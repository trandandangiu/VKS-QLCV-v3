// src/components/tp/TpHeroHeader.tsx
import React from 'react';
import { Building, User, Calendar, Clock, TrendingUp, RefreshCw } from 'lucide-react';

interface TpHeroHeaderProps {
    userName: string;
    roomCode?: string;
...
export const TpHeroHeader: React.FC<TpHeroHeaderProps> = ({
    userName,
    roomCode,
    pvtManagerName,
    summary,
        ...
        >
            ...
            <div className="absolute inset-0 opacity-10 pointer-events-none">
                <svg width="100%" height="100%">
                    <defs>
                        <pattern id="dots-tp-hero" x="0" y="0" width="24" height="24" patternUnits="userSpaceOnUse">
                            <circle cx="2" cy="2" r="1.5" fill="white" />
                        </pattern>
                    </defs>
                    <rect width="100%" height="100%" fill="url(#dots-tp-hero)" />
                </svg>
            </div>
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-3 flex-wrap">
                            ...
                            {pvtManagerName && (
                                <span className="px-2.5 py-1 bg-emerald-500/25 backdrop-blur-sm rounded-full border border-emerald-400/40 text-[10px] font-bold text-emerald-100">
                                    PVT: {pvtManagerName}
                                </span>
                            )}
                        </div>
{/* 
                    ...
                    <div className="flex flex-col gap-2.5 shrink-0">
                        ...
                        <div className="grid grid-cols-3 gap-2.5">
                            <KpiBadge
                                label="Chờ xử lý"
                                value={summary.choXuLy || 0}
                                color="bg-amber-500/25 border-amber-400/40"
                                alert={(summary.choXuLy || 0) > 0}
                            />
                            <KpiBadge
                                label="Đang xử lý"
                                value={summary.dangXuLy || 0}
                                color="bg-blue-500/25 border-blue-400/40"
                            />
                            {onRefresh && (
                                <button
                                    onClick={onRefresh}
